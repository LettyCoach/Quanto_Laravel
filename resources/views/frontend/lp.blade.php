...
<head>
    ...
    <!-- Slick CSS -->
    <link rel="stylesheet" type="text/css" href="/public/lib/slick/css/slick-theme.css">
    <link rel="stylesheet" type="text/css" href="/public/lib/slick/css/slick.css">
    ...
    <script src="/public/lib/slick/js/slick.min.js"></script>
    ...
    <script>
        $(document).ready(function () {
            $('#icon-size-select').on('change', function () {
                ...
                $('.desc-wrapper,.prof-img,#desctxtarea,#prof-label').removeClass('size1 size2').addClass('size' + selectedValue);
            });
        });
    </script>
    <!-- //アイコンサイズ調整 -->
    ...
    <script>
        $(document).ready(function () {
            $('#icon-position-select').on('change', function () {
                var selectedValue = $(this).val();
                $('.desc-wrapper,.prof-img,#desctxtarea,#prof-label').removeClass('posi1 posi2').addClass('posi' + selectedValue);
            });
        });
    </script>
    <!-- //アイコン位置調整 -->
...
@endif
        ...
        <div id="title-id-wrapper">
            ...
            <div class="d-flex justify-content-around align-items-center prof-area">
                <div class="d-flex align-items-center prof-area-inner">
                    @if ($isEdit)
                    <label for="profile_url" id="prof-label" class="prof-label size{{str_replace('"','',$icon_size)}} posi{{str_replace('"','',$icon_position)}}">
                        ...
                        <img src="{{ $profile_url != '' ? asset($profile_url) : asset('public/img/lp-tmp/default_profile.png') }}"
                             class="pointer prof-img  size{{str_replace('"','',$icon_size)}} posi{{str_replace('"','',$icon_position)}}" alt="プロフィール画像">
                    </label>
                    @else
                        <span id="prof-label" class="prof-label size{{str_replace('"','',$icon_size)}} posi{{str_replace('"','',$icon_position)}}">
                            <img src="{{ $profile_url != '' ? asset($profile_url) : asset('public/img/lp-tmp/default_profile.png') }}"
                             class="pointer prof-img  size{{str_replace('"','',$icon_size)}} posi{{str_replace('"','',$icon_position)}}" alt="プロフィール画像">
                        </span>
                    @endif
                    <div class="desc-wrapper size{{str_replace('"','',$icon_size)}} posi{{str_replace('"','',$icon_position)}}">
                        <div class="desc-wrapper-inner">
                            @if ($isEdit)
                                <textarea id="desctxtarea" class=" size{{str_replace('"','',$icon_size)}} posi{{str_replace('"','',$icon_position)}}" name="description" style="resize: auto;"
                                          placeholder="自己紹介">{{ $lp->description }}</textarea>
                            @else
                        ...
                        @if ($isEdit)
                            <div class="size-posi-select">
                                <select id="icon-size-select" name="icon_size">
                                    <option value="1"{{str_replace('"','',$icon_size) == 1 ? ' selected' : ''}}>小サイズ</option>
                                    <option value="2"{{str_replace('"','',$icon_size) == 2 ? ' selected' : ''}}>大サイズ</option>
                                </select>
                                <select id="icon-position-select" name="icon_position">
                                    <option value="1"{{str_replace('"','',$icon_position) == 1 ? ' selected' : ''}}>パターンA</option>
                                    <option value="2"{{str_replace('"','',$icon_position) == 2 ? ' selected' : ''}}>パターンB</option>
                                </select>
                            </div>
                        @endif
        ...
        <main style="margin-top: 0">
            <section class="first-sec size{{str_replace('"','',$icon_size)}} posi{{str_replace('"','',$icon_position)}}">
                    ...
                    <div class="row contents">
                        ...
                        <div class="image-slider-wrapper">
                                ...
                                <div class="slider-area">
                                        ...
                                        @foreach ($contents as $content)
                                            @if($content->type == 1)
                                                <li class="content-wrapper" data-content-id="{{ $content->id }}">
                                                    ...
                                                    @if ($isEdit)
                                                        <input type="hidden" name="contents[{{ $content->id }}][type]" value="1">
                                                        <button type="button" class="btn btn-danger position-absolute" onclick="deleteContent(event)" style="height: unset; right: 0;"><i class="fa fa-times"></i></button>
                                                        <input type="hidden" name="contents[{{ $content->id }}][url]" value="{{ $file_url }}">
                                                        <input type="file" id="content_file_{{ $content->id }}" class="" onchange="input_type_file(event)">
                                                    @endif
                                                    <img src="{{ asset($file_url) }}" alt="商品画像" class="img-thumbnail" data-bs-toggle="modal"
                                                         data-bs-target="#myModal">
                                                    @if ($isEdit)
                                    ...
                                    @if ($isEdit)
                                        <div class="row justify-content-center">
                                            <button type="button" class="btn-add-content col span-2" data-type="1">追加</button>
                                        </div>
                                    @endif
                                </div>
                            @endif
                            ...
                            @else
                                    ...
                                    @foreach ($contents as $content)
                                        @if($content->type == 2)
                                            <li class="content-wrapper" data-content-id="{{ $content->id }}">
                                                ...
                                                $description = isset($content->description) ? $content->description : ''; ?>
                                                @if ($isEdit)
                                                        <input type="hidden" name="contents[{{ $content->id }}][type]" value="2">
                                                    <button type="button" class="btn btn-danger position-absolute" onclick="deleteContent(event)" style="height: unset; right: 0;"><i class="fa fa-times"></i></button>
                                                    <input type="hidden" name="contents[{{ $content->id }}][url]" value="{{ $file_url }}">
                                                    <input type="file" id="content_file_{{ $content->id }}" class="" onchange="input_type_file(event)">
                                                @endif
                                ...
                                @if ($isEdit)
                                    <div class="row justify-content-center">
                                        <button type="button" class="btn-add-content col span-2" data-type="2">追加</button>
                                    </div>
                                @endif
                            </div>
                            @endif
                            ...
                            @else
                                    ...
                                    @foreach ($contents as $content)
                                        @if($content->type == 3)
                                            <li class="content-wrapper" data-content-id="{{ $content->id }}">
                                                ...
                                                $description = isset($content->description) ? $content->description : ''; ?>
                                                @if ($isEdit)
                                                        <input type="hidden" name="contents[{{ $content->id }}][type]" value="3">
                                                        <button type="button" class="btn btn-danger position-absolute" onclick="deleteContent(event)" style="height: unset; right: 0;"><i class="fa fa-times"></i></button>
                                                    <input type="hidden" name="contents[{{ $content->id }}][url]" value="{{ $file_url }}">
                                                    <input type="file" id="content_file_{{ $content->id }}" class="" onchange="input_type_file(event)">
                                                @endif
                                                <img src="{{ asset($file_url) }}" alt="商品画像" class="img-thumbnail layout-2-item" data-bs-toggle="modal"
                                                     data-bs-target="#myModal">
                                                @if ($isEdit)
                                                    <textarea name="contents[{{ $content->id }}][description]" placeholder="ここに説明文が入ります">{{ $description }}</textarea>
                                                @elseif(!empty($description))
                                ...
                                @if ($isEdit)
                                    <div class="row justify-content-center">
                                        <button type="button" class="btn-add-content col span-2" data-type="3">追加</button>
                                    </div>
                                @endif
                            </div>
                            @endif
            ...
            @else
                ...
                <div class="d-flex justify-content-around align-items-center">
                    <div class="d-flex align-items-center">
                        @if ($isEdit)
                            <textarea name="shop_info" style="resize: auto;"
                                      placeholder="ショップ等の情報">{{ $shop_info }}</textarea>
                        @else
                            {{ $shop_info }}
                        @endif
                    </div>
                            ...
                            <div class="d-flex">
                                <div class="d-flex justify-content-center align-items-center flex-wrap mb-3">
                                    <a href="http://twitter.com/{{ $twitter_url }}" target="_blank"><img
                                                src="{{ asset('public/img/lp-tmp/Twitter_logo.png') }}"
                                                class="sns-icons" alt="twitter"></a>
            ...
            <section>
                <div class="d-flex justify-content-around align-items-center">
                    <div class="d-flex align-items-center">
                        @if ($isEdit)
                            <textarea name="shop_intro" style="resize: auto;"
                                      placeholder="ショップ等の紹介">{{ $shop_intro }}</textarea>
                        @else
                            {{ $shop_intro }}
                        @endif
                    </div>
            ...
            @else
                ...
                <div class="container">
                    ...
                    <div id="dispmap">
                        ...
                        @else
                            <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3240.5000881778465!2d139.6898348270166!3d35.68930947258462!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x60188cd4b71a37a1%3A0xf1665c37f38661e8!2z5p2x5Lqs6YO95bqB!5e0!3m2!1sja!2sjp!4v1681100126754!5m2!1sja!2sjp" width="600" height="450" style="border:0;" allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
                        @endif
                    </div>
                </div>
            </section>
