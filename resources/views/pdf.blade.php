...
<body>
    ...
    <table>
        ...
        <tr>
            ...
            <td><div style="border: solid 1px #dbdbdb; text-align:center;"><font size="5">{{ $purpose }}</div></td>
    ...
    <?php
        ...
        <?php } else {
            $_pages = 1 + ceil(($_count - 10) / 14);
            $_new_page = 10; ?>
            <div style="text-align:center; padding-top:5px"><font size="4">内容明細 1/{{ $_pages }}</div>
        <?php }
    ?>
    <!--内容明細１ページ目-->
    <table>
        ...
        <?php for ($i = 0; $i < $_new_page; $i++) {
            ...
            if (empty($_quantities[$i]) || $_prices[$i] == 0){    
                $_quantities[$i] = 0;
            } 
            if ($_taxes[$i] == 10){
    ...
    <?php if ($_pages > 1){ ?>
        <table style="padding-top:5px">
            ...
            <tr>
                <td><div style="text-align:center; padding:right 50px"><font size="4">小計</font></div></td>
                <td><div style="text-align:center;"><font size="4">{{ $_subtotal_count }}点</div></td>
                <td><div style="text-align:right;"><font size="5">{{ number_format($_subtotal_price) }}</div></td>
            </tr>
        </table>        
        <hr class="hr1">
        ...
        <?php for($p = 2; $p <= $_pages; $p++) { 
            ...
            <table>
                ...
                <tr>
                    <td><div><img src="{{url('public/img/pdf_logo.png')}}" style="height:50;"></div></td>
                    <td><div style="border: solid 1px #dbdbdb; text-align:center;"><font size="5">{{ $purpose }}</div></td>
                    <td><div style="text-align:right;"><font size="2">発行日：{{ $date }}</div></td>
                </tr>
            </table>
            <hr class="hr1">
            <div style="text-align:center; padding-top:5px"><font size="4">内容明細 {{ $p }}/{{ $_pages }}</div>
            <table>
                <tr>
                    <th width="280">
                    <th width="120">
                ...
                <tr>
                    <td class="td1"><div style="text-align:center;">内容</div></td>
                    <td class="td1"><div style="text-align:center;">数量</div></td>
                    <td class="td1"><div style="text-align:center;">金額(円)</div></td>
                </tr>
                <?php for ($i = $_i_save; $i < $_new_page; $i++) { 
                    if (empty($_prices[$i])){    
                        $_prices[$i] = 0;
                    }
                    if (empty($_quantities[$i]) || $_prices[$i] == 0){    
    ...
    <div style="border: solid 1px #dbdbdb; width: 100%;">
        <p style="margin: 0; height: 38px"><font color="#dbdbdb">(備考)</font>{{ $note }}<p>
    </div>
    <div style="position:absolute; bottom: 0; left: 0; right: 0; text-align:center;"><img src="{{url('public/img/footer_logo.png')}}" height="30"></div>    
</body>
<?php
