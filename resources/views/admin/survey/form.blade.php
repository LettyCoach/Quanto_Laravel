...
@section('js')
    <script>
        ...
        let count_questions = questions.length;
    </script>
...
@section('main-content')
    ...
    <div class="row">
        <div class="col-8">
            <p id="retOp"></p>
            <form class="" id="survey" method="post" target="myResult" action="{{ route('admin.survey.save') }}"
                enctype="multipart/form-data">
                @csrf
                <div class="row" style="align-items: baseline;">
                    <input id="saveSurvey" type="submit" class="btn btn-primary float-left" value="保存"
                    ...
                    <div class="d-flex mb-3" style="width:350px">
                        ...
                        <div id="cardPayment" class="switch_box box_1 d-flex align-items-center">
                            <input type="checkbox" class="switch_1" id="paymentStatus" name="payment_status"
                                {{ isset($survey['payment_status']) && $survey['payment_status'] == 1 ? 'checked' : '' }}>
                        </div>
                    </div>
                    <label class="mr-3 col-form-label" for="">利用方法: </label>
                    <div>
                        <?php $how2use = isset($surveySettings->how2use) ? $surveySettings->how2use : '0'; ?>
                        <input type="radio" value="0" id="how2useEstimate" name="how2use"
                            {{ $how2use == 0 ? 'checked' : '' }} />
                        <label for="how2useEstimate">自動見積もり</label>
                        <input type="radio" value="1" id="how2useInvoice" name="how2use"
                            {{ $how2use == 1 ? 'checked' : '' }} />
                ...
                <div class="row">
                    ...
                    <span class="col-md-8 d-flex align-items-center">
                        <input class="form-control" type="text" placeholder="https://..." name="linkUrl"
                            value="{{ isset($surveySettings->linkUrl) ? $surveySettings->linkUrl : '' }}">
                    </span>
                </div>
                    ...
                    <div class="col-md-8 align-items-center">
                        <div class="row">
                            <div class="col-md-12 d-flex align-items-center">
                                ...
                                <input class="" type="color" name="answer_char_color"
                                    value="{{ isset($survey['answer_char_color']) ? $survey['answer_char_color'] : '#000000' }}">
                                <label class="col-form-label mx-3">回答選択:</label>
                                <input class="" type="color" name="answer_selected_border_color"
                                    value="{{ isset($survey['answer_selected_border_color']) ? $survey['answer_selected_border_color'] : '#ffffff' }}">
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-12 d-flex align-items-center">
                                ...
                                <select class="form-control" name="status">
                                    @foreach ($statuses as $status)
                                        <option value="{{ $status->id }}"
                                            {{ $status->id == $survey['status'] ? 'selected' : '' }}>{{ $status->name }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-6 d-flex align-items-center">
                        <input type="file" name="profile_path" onchange="loadFile(event, '#show_logo img')"
                            data-showimage="#show_log img" accept="image/png, image/gif, image/jpeg">
                    </div>
                </div>
                <div class="form-group row">
                    ...
                    <div class="col-md-4">
                        <select name="purpose" class="form-control">
                            ...
                            @foreach ($purposes as $key => $item)
                                <option value="{{ $item->name }}" {{ $item->name == $purpose ? 'selected' : '' }}>
                                    {{ $item->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-md-4">
                        <select name="member" class="form-control">
                            ...
                            @foreach ($members as $key => $item)
                                <option value="{{ $item->name }}" {{ $item->name == $member ? 'selected' : '' }}>
                                    {{ $item->name }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="form-group row">
                        ...
                        <select name="payment_method" class="form-control">
                            ...
                            @foreach ($payment_methods as $key => $item)
                                <option value="{{ $item->name }}"
                                    {{ $item->name == $payment_method ? 'selected' : '' }}>{{ $item->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4" id="multiPay"
                        @if (isset($survey['payment_status']) && $survey['payment_status'] == 1) style="display:block;" @else style="display:none;" @endif>
                        <?php $pay_methods = isset($surveySettings->pay_methods) ? json_decode($surveySettings->pay_methods) : []; ?>
                    ...
                    <div class="col-md-6 d-flex">
                        ...
                        <div class="switch_box box_1 d-flex align-items-center">
                            ...
                            <input type="checkbox" class="switch_1" name="use_inputform"
                                {{ $use_inputform == 1 ? 'checked' : '' }}>
                        </div>
                    </div>
                </div>
                <div class="form-group row">
                    <div class="col-md-6" id="deliveryMethods"
                        @if (isset($survey['payment_status']) && $survey['payment_status'] == 1) style="display:block;" @else style="display:none;" @endif>
                        <div class="row">
                            <label class="col-md-4 col-form-label" for="">受け取り方法: </label>
                            <?php $delivery_methods = isset($surveySettings->delivery_methods) ? json_decode($surveySettings->delivery_methods) : []; ?>
                            <div class="col-md-8">
                                <select name="delivery_methods[]" class="form-control selectpicker" multiple
                                    placeholder="選択なし">
                                    ...
                                    <option value="デリバリー" @if (in_array('デリバリー', $delivery_methods) == true) selected @endif>デリバリー
                                    </option>
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <input class="form-control" type="text" placeholder="円" name="totalPrefix"
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-4 d-flex align-items-center">
                        ...
                        <input type="checkbox" class="switch_1" {{ $autoSendMail == 1 ? 'checked' : '' }}
                            name="autoSendMail" />
                    </div>
                    ...
                    <div class="col-md-4 d-flex align-items-center">
                        ...
                        <input type="radio" value="0" id="widgetAlignLeft" name="widgetAlign"
                            {{ $widget_align == 0 ? 'checked' : '' }} />
                        <label class="m-0" for="widgetAlignLeft">左寄せ</label>
                        <input type="radio" value="1" id="widgetAlignRight" name="widgetAlign"
                            {{ $widget_align == 1 ? 'checked' : '' }} />
                        <label class="m-0" for="widgetAlignRight">右寄せ</label>
                    </div>
                </div>
                <input type="hidden"
                    value="{{ isset($surveySettings->displayQrcode) ? $surveySettings->displayQrcode : 0 }}"
                    name="disp_qrcode">
                <div class="form-group row">
                            ...
                            <div class="ui-state-default">
                                <div id="question_{{ $question->id }}_wrapper">
                                    <section class="accordion">
                                        ...
                                        <label id="label-{{ $question->id }}" class="ac_Label"
                                            for="block-{{ $question->id }}">質問{{ $index + 1 }}：{{ $question->title }}</label>
                                        <div id="content-{{ $question->id }}" class="ac_content">
                                            <div class="question" id="question_{{ $question->id }}">
                                                ...
                                                <input type="hidden" value="{{ $q_required }}"
                                                    name="questions[q_{{ $q_index }}][required]">
                                                <input type="hidden" value="{{ $q_limit }}"
                                                    name="questions[q_{{ $q_index }}][limit]">
                                                <div class="row form-group ">
                                                    <label
                                                        class="col-md-2 pl-1 col-form-label d-flex align-items-center question-index-{{ $index }}">質問{{ $index + 1 }}:</label>
                                                    <div class="col-md-8">
                                                    ...
                                                    <div class="col-md-1 d-flex align-items-center">
                                                        <button type="button" class="btn btn-danger buttonDelete"
                                                            onclick="onDeleteQuestion('{{ $q_index }}')">
                                                            <i class="fa fa-times"></i>
                                                        </button>
                                                        <button type="button" class="btn btn-primary buttonEdit"
                                                            style="display: block"
                                                            onclick="onEdit('question_{{ $q_index }}')"><i
                                                                class="fa fa-pen"></i></button>
                                                    </div>
                                                </div>
                                                <div class="row form-group ">
                                                    <label
                                                        class="ml-2 pl-1 col-form-label d-flex align-items-center">質問コード</label>
                                                    <div class="col-md-8">
                                                        <input class="question_code" type="text"
                                                            value="{{ json_decode($question->settings, true)['question_code'] }}"
                                                            name="questions[q_{{ $q_index }}][question_code]" />
                                                    </div>
                                                </div>
                                                <div class="row form-group ">
                                                    <label
                                                        class="ml-2 pl-1 col-form-label d-flex align-items-center">親カテゴリ</label>
                                                    <div class="col-md-8">
                                                        <select class="form-control parentCategory" disabled
                                                            name="questions[q_{{ $q_index }}][parent_category]">
                                                            <option value="{{ $parentCategory }}"
                                                                {{ $parentCategory != 'なし' ? 'selected' : '' }}>
                                                                {{ $parentCategory }}{{ $parentTitle }}</option>
                                                        </select>
                                                    </div>
                                                </div>
                                                <div class="row form-group ">
                                                    <label
                                                        class="ml-2 pl-1 col-form-label d-flex align-items-center">関連情報</label>
                                                    <div class="col-md-8">
                                                        <select class="form-control questionReferralInfo" disabled
                                                            name="questions[q_{{ $q_index }}][referral_info]">
                                                            <option value=""></option>
                                                            @foreach ($referral_info as $ref)
                                                                <option value="{{ $ref->id }}"
                                                                    {{ $ref->id === $question->referral_info ? 'selected' : '' }}>
                                                                    {{ $ref->name }}</option>
                                                            @endforeach
                                                        </select>
                                                    </div>
                                                ...
                                                <div class="row form-group">
                                                    <label
                                                        class="ml-2 pl-1 col-form-label d-flex align-items-center">回答整列</label>
                                                    <div class="col-md-8">
                                                        <select class="form-control" disabled
                                                            name="questions[q_{{ $q_index }}][answer_align]"
                                                            id="questionAnswerAlign_{{ $q_index }}">
                                                            <option value="0"
                                                                {{ $question->answer_align == 0 ? 'selected' : '' }}>左寄せ
                                                            </option>
                                                            <option value="1"
                                                                {{ $question->answer_align == 1 ? 'selected' : '' }}>センター
                                                            </option>
                                                            <option value="2"
                                                                {{ $question->answer_align == 2 ? 'selected' : '' }}>右寄せ
                                                            </option>
                                                        </select>
                                                    </div>
                                                </div>
                                                <div class="row form-group">
                                                    <label
                                                        class="ml-2 pl-1 col-form-label d-flex align-items-center">数量の選択</label>
                                                    <div class="col-md-8">
                                                        <input type="checkbox" class="switch_1" disabled
                                                            {{ $selectQuantity == 1 ? 'checked' : '' }}
                                                            name="questions[q_{{ $q_index }}][select_quantity]"
                                                            id="questionSelectQuantity_{{ $q_index }}">
                                                    </div>
                                                </div>
                                                <div class="row show_img{{ $q_index }}">
                                                    ...
                                                    <?php } else { ?>
                                                    <img src="" class="col fs-question-image mb-2"
                                                        style="display: none">
                                                    <?php } ?>
                                                </div>
                                                <div class="row form-group">
                                                    <div class="col-md">
                                                        <input
                                                            onchange="loadFile(event, '.modal-body .show_img{{ $q_index }} img')"
                                                ...
                                                <div class="d-flex mb-1">
                                                    <div id="answers_{{ $q_index }}"
                                                        class="d-flex answerDropArea flex-wrap"
                                                        ondrop="dropAnswer(event)">
                                                        @php
                                                            $parents = [['id' => 0, 'title' => '']];
                                    ...
                                    ?>
                                                <div class="answer card mr-2 mb-2" id="_answer_{{ $answer->id }}">
                                                    <div class="card-header d-flex justify-content-between p-2"
                                                        ondrop="dropAnswerOption(event)">
                                                        @if ($answer->type == 1)
                                                            <span>回答 テキスト</span>
                                                        @else
                                                            <span>回答</span>
                                                        @endif
                                                        <button class="text-danger buttonDeleteAnswer" type="button"
                                                            onclick="onDelete('_answer_{{ $answer->id }}')"><i
                                                                class="fa fa-times"></i></button>
                                                    </div>
                                                    <div class="card-media" id="_answer_option_{{ $q_index }}"
                                                        ondrop="dropAnswerOption(event)">
                                                        オプション：
                                                        @if ($a_option == '0')
                                                            <span>無し</span>
                                                        @else
                                                            ...
                                                            <button type="button" class="text-danger buttonDeleteAnswer"
                                                                onclick="onDeleteAnswerOption('_answer_option_{{ $q_index }}')"><i
                                                                    class="fa fa-times"></i></button>
                                                        @endif
                                                        <input type="hidden" value="{{ $a_option }}"
                                                            name="questions[q_{{ $q_index }}][answer_option]">
                                                    </div>
                                                    <div class="card-body p-2">
                                                        <input type="hidden" value="{{ $answer->id }}"
                                                            name="questions[q_{{ $q_index }}][answers][a_{{ $answer->id }}][id]">
                                                        <input type="hidden" value="{{ $answer->type }}"
                                                            name="questions[q_{{ $q_index }}][answers][a_{{ $answer->id }}][type]">

                                                        <textarea placeholder="回答" class="form-control"
                                                            name="questions[q_{{ $q_index }}][answers][a_{{ $answer->id }}][title]" required>{{ $answer->title }}</textarea>
                                                        @if (count($parents) > 1 && $answer->type != 3)
                                                            集団
                                                            <select class="form-control mt-1"
                                                                name="questions[q_{{ $q_index }}][answers][a_{{ $answer->id }}][parent_id]">
                                                        ...
                                                        @if (count($next_questions) > 1)
                                                            次の問題
                                                            <select class="form-control mt-1"
                                                                name="questions[q_{{ $q_index }}][answers][a_{{ $answer->id }}][next_question_id]">
                        ...
                        @if (($answer->type == 4 || $answer->type == 5) && $answer->question_id == $question->id)
                            @if (!$hasRadio)
                                <div class="answer card mr-2 mb-2" id="_answer_{{ $answer->id }}">
                                    <div class="card-header d-flex justify-content-between p-2"
                                        ...
                                        @endif
                                        <button class="text-danger buttonDeleteAnswer" type="button"
                                            onclick="onDelete('_answer_{{ $answer->id }}', this)"><i
                                                class="fa fa-times"></i></button>
                                    </div>
                                    <div class="card-media" id="_answer_option_{{ $q_index }}"
                                        ondrop="dropAnswerOption(event)">
                            ...
                            <div class="col-6 p-1" id="_answer_{{ $answer->id }}_sub_{{ $aCount }}">
                                ...
                                <input type="hidden" value="{{ $answer->type }}"
                                    name="questions[q_{{ $q_index }}][answers][a_{{ $answer->id }}][type]">
                                <button type="button" class="text-danger buttonDeleteAnswer"
                                    onclick="onDelete('_answer_{{ $answer->id }}_sub_{{ $aCount }}')"><i
                                        class="fa fa-times"></i></button>
                                <label>回答{{ $aCount }}</label>
                                <input placeholder="回答" class="form-control"
                                    name="questions[q_{{ $q_index }}][answers][a_{{ $answer->id }}][title]"
                                    required value="{{ $answer->title }}" />
                                <label>税込価格</label>
                                <input placeholder="税込価格" class="form-control"
                                    name="questions[q_{{ $q_index }}][answers][a_{{ $answer->id }}][value]"
                                    required value="{{ $answer->value }}" />
                                <label>税率</label>
                                <select class="form-control tax" disabled
                                    name="questions[q_{{ $q_index }}][answers][a_{{ $answer->id }}][tax]">
                                    <option value=""></option>
                                    <option value="8" {{ $answer->tax == 8 ? 'selected' : '' }}>8%</option>
                                    <option value="10" {{ $answer->tax == 10 ? 'selected' : '' }}>10%</option>
                                </select>
                                <label>関連情報</label>
                                <select class="form-control referralInfo" disabled
                                    name="questions[q_{{ $q_index }}][answers][a_{{ $answer->id }}][referral_info]">
                                    <option value=""></option>
                                    @foreach ($referral_info as $ref)
                                        <option value="{{ $ref->id }}"
                                            {{ $ref->id == $answer->referral_info ? 'selected' : '' }}>
                                            {{ $ref->name }}</option>
                                    @endforeach
                                </select>
                                ...
                                <div class="row form-group">
                                    <div class="col-md">
                                        <input
                                            onchange="loadFile(event, '.modal-body .show_img_{{ $q_index }}_{{ $answer->id }} img')"
                                            accept="image/png, image/gif, image/jpeg" type="file" class="form-control"
                                ...
                                @if (count($next_questions) > 1)
                                    次の問題
                                    <select class="form-control mt-1 next_question_id"
                                        name="questions[q_{{ $q_index }}][answers][a_{{ $answer->id }}][next_question_id]">
                                        @foreach ($next_questions as $n_item)
                                            <option value="{{ $n_item['id'] }}"
                                                {{ $n_item['id'] == $answer->next_question_id ? 'selected' : '' }}>
    ...
    <div class="while-btn-add-survey-wrapper">
        <button type="button" class="btn-primary btn-add-survey while-btn-add-survey" data-toggle="modal"
            data-target="#modalAddQuestion">
            +
        </button>
    </div>
    </div>
    ...
    <div id="btn-add-survey-area">
        <button id="btn-add-survey" class="btn btn-primary btn-add-survey" onclick="addNewQuestion()">
            +
        </button>
    </div>
    </div>
    ...
    <div class="col-4">
        ...
        <div class="card">
            <h6 class="form-control-label m-3">
                <?php
                $urlFront = $cartHost . $survey['token'];
                $displayFront = 'block';
                $urlShow = "$clientHost?id=" . $survey['token'];
                ...
                ?>
                <div id="urlFront" style="display: {{ $displayFront }}">
                    URL: <a href="{{ $urlFront }}" target="_blank">{{ $urlFront }}</a>
                    <br><br>
                    <?php $qrCode = \SimpleSoftwareIO\QrCode\Facades\QrCode::size(100)->generate($urlFront); ?>
                </div>
                <div id="urlShow" style="display: {{ $displayShow }}">
                    URL: <a href="{{ $urlShow }}" target="_blank">{{ $urlShow }}</a>
                    <br><br>
                    <?php $qrCode = \SimpleSoftwareIO\QrCode\Facades\QrCode::size(100)->generate($urlShow); ?>
                </div>

                {{-- @if (isset($survey['payment_status']) && $survey['payment_status'] == 1)
                    URL: <a href="<?php echo $cartHost; ?>{{ $survey['token'] }}"
                        target="_blank"><?php echo $cartHost; ?>{{ $survey['token'] }}</a>
                    <br><br>
                    <?php $qrCode = \SimpleSoftwareIO\QrCode\Facades\QrCode::size(100)->generate($cartHost . $survey['token']); ?>
                @else
                    URL: <a href="<?php echo $clientHost; ?>?id={{ $survey['token'] }}"
                        target="_blank"><?php echo $clientHost; ?>?id={{ $survey['token'] }}</a>
                    <br><br>
                ...
                <div class="row align-items-center">
                    ...
                    <div class="switch_box box_1 d-flex">
                        <input type="checkbox" class="switch_1" name="disp_qrcode_dummy"
                            {{ isset($surveySettings->displayQrcode) && $surveySettings->displayQrcode == 1 ? 'checked' : '' }}>
                    </div>
                </div>
            </h6>
        ...
        <div class="card">
            ...
            <div class="embbed-html">
                <div>
                    <a class="clipboard iframe-html-clipboard" style="cursor: pointer;" title="コピー"
                        data-clipboard-text="<?php echo $iFrameSource; ?>"><i class="fa fa-clipboard mr-1"></i>コピーします</a>
                </div>
                <div class="iframe-html"><?php echo $iFrameSource; ?></div>
            </div>
            <div class="embbed-html">
                <div>
                    <a class="clipboard" style="cursor: pointer;" title="コピー"
                        data-clipboard-text="<?php echo $fullSource; ?>"><i class="fa fa-clipboard mr-1"></i>コピーします</a>
                </div>
                <?php echo $fullSource; ?>
            </div>
            <?php
